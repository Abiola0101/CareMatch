...
import type {
  CareModeRow,
  HospitalPrivilegeRow,
  MatchResultDetail,
  PatientCaseDetail,
} from "@/lib/patient/case-detail";

export type InsurerCaseDetailView = PatientCaseDetail & {
  case: PatientCaseDetail["case"] & {
    policyholder_ref: string | null;
    share_token: string;
  };
};

async function buildMatchesFromRows(
  ...
  const specIds = Array.from(new Set(matchesList.map((m) => m.specialist_id)));
  ...
  const { data: careModeRows } = await admin
    ...
    .select("specialist_id, mode, available, detail, fee_range, wait_days")
...
function mapInsurerCaseRow(c: Record<string, unknown>): InsurerCaseDetailView["case"] {
  return {
    id: c.id as string,
    specialty: (c.specialty as string) ?? null,
    title: null,
    condition_summary: (c.condition_summary as string) ?? null,
    duration_months: null,
    urgency: (c.urgency as string) ?? null,
    diagnosis_status: null,
    additional_notes: null,
    investigations_done: (c.investigations_done as string[]) ?? null,
    treatments_tried: (c.treatments_tried as string) ?? null,
    status: (c.status as string) ?? "submitted",
    created_at: c.created_at as string,
    policyholder_ref: (c.policyholder_ref as string) ?? null,
    share_token: (c.share_token as string) ?? "",
  };
}

export async function loadInsurerCaseDetail(
  caseId: string,
): Promise<InsurerCaseDetailView | null> {
  ...
  const { data: matchRows } = await admin
    ...
    .select("*")
    .eq("insurer_case_id", caseId)
    .order("rank_position", { ascending: true });

  const matchesList = matchRows ?? [];
  const matches =
    matchesList.length === 0 ? [] : await buildMatchesFromRows(admin, matchesList);

  return {
    case: mapInsurerCaseRow(c as Record<string, unknown>),
    matches,
  };
}

export async function loadInsurerCaseDetailByShareToken(
  token: string,
): Promise<InsurerCaseDetailView | null> {
  const admin = createServiceRoleClient();

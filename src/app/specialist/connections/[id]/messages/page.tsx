...
export default async function SpecialistConnectionMessagesPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  if (ce || !conn) notFound();
  if (conn.specialist_id !== user.id) notFound();
  if (conn.status !== "accepted") {
    redirect("/specialist/connections");
  }
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Conversation with {peer?.full_name ?? "patient"}.
      </p>
      ...
      <div className="mt-8">
        <ConnectionThread
          ...
          peerLabel={peer?.full_name ?? "Patient"}
        />
      </div>
    </main>
  );
